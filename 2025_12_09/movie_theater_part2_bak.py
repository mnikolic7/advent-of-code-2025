...
def rectangle_is_valid(point1,point2,X=None,Y=None):
    ...
    for i in range(N):
        ...
        q1=is_point_in_rect(curr_point, point1, point2)
        q2=is_point_in_rect(next_point, point1, point2)
...
def get_opposite_corners(point1,point2):
    x1,y1=point1
    x2,y2=point2
...
def get_all_corners(point1, point2):
    x1,y1=point1
    x2,y2=point2
    #bottom left 
    #I am thinking here with y axis going up, not downwards like in the problem statement.
    ...
    #bottom right
    br=(max(x1,x2),min(y1,y2))
    #top right
    tr=(max(x1,x2),max(y1,y2))
    #top left
    tl=(min(x1,x2),max(y1,y2))
...
def is_point_in_rect(point, rect_point1, rect_point2):
    x1,y1=rect_point1
    x2,y2=rect_point2

    cond_x = point[0]>=min(x1,x2) and point[0]<=max(x1,x2)
    cond_y = point[1]>=min(y1,y2) and point[1]>=max(y1,y2)
    ...
    for i in range(N):
        for j in range(i+1,N):
            ...
            for k in range(N):
                ...
                q1=is_point_in_rect(curr_pt, point1, point2)
                q2=is_point_in_rect(next_pt, point1, point2)

                plt.title(f'point 1 is in?={q1}, and next is in?={q2}')
                pt3.set_data([curr_pt[0]+0.5],[curr_pt[1]+0.5])                
